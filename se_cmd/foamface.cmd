...
foam_signature := {
  ...
  printf "[";
  for ( inx := 1 ; inx <= body_count; inx += 1) {
	printf "[";
  for ( iny := 1 ; iny <= max_poly_sides-1; iny += 1) {
	printf "%d,",body_poly_counts[inx][iny];
	};
	if inx<body_count then
		printf "%d],\n",body_poly_counts[inx][iny]
	else
		printf "%d]]",body_poly_counts[inx][iny];
	};
