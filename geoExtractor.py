...
def loadGeoFoamFile(geofile):
    ...
    geofile_stream = open(geofile_path, "r")
...
def FitTheBox(vertices,edges,surfaces,volumes):
    nvertices=[]
    nedges=[]
    nsurfaces=[]
    nvolumes=[]
...
def clipCells(centers,vertices,edges,surfaces,volumes,nE,nS):
    ...
    geofile = os.path.join(mypath, 'clipFoam.geo')
    fg = open(geofile, 'w')
...
def writeAsGeo(vertices,edges,surfaces,volumes,filename):
    geofile = os.path.join(mypath, filename)
    out = open(geofile, 'w')
